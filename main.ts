...
function isHitOrMiss (enemyBoats: Sprite[][]) {
    for (let index = 0; index <= 2; index++) {
        for (let previousBoatSprite2 of enemyBoats[index]) {
            if (grid.spriteCol(previousBoatSprite2) == grid.spriteCol(cursor) && grid.spriteCol(previousBoatSprite2) == grid.spriteCol(cursor)) {
